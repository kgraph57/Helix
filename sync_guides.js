...
const sourceDir = path.join(__dirname, 'client', 'guides');
const targetDirs = [
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (fs.existsSync(sourcePath) && fs.statSync(sourcePath).isDirectory()) {
        ...
        console.log(`  ⚠️  スキップ: ${path.relative(__dirname, targetPath)} (ソースに存在しないが、他のガイド用の可能性があるため保持)`);
      }
    } else if (entry.isFile()) {
      // 重複ファイルを削除
      if (shouldSkipFile(entry.name)) {
        fs.unlinkSync(targetPath);
        console.log(`  🗑️  削除: ${path.relative(__dirname, targetPath)} (重複ファイル)`);
      }
    }
  }
}

console.log('🔄 client/guides/ディレクトリを統合・同期中...\n');
console.log('📋 ソース: client/guides/\n');

for (const targetDir of targetDirs) {
  const relativeTarget = path.relative(__dirname, targetDir);
...
console.log('✨ 統合・同期完了！');
console.log('\n💡 client/guides/ディレクトリが唯一のソースです。');
console.log('   編集は client/guides/ で行い、npm run sync:guides で同期してください。');
